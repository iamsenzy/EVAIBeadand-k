...
void TableEditorWidget::removeButton_Clicked()
{
    ...
    {
        if(this->windowTitle()== "Esemenyek szerkesztése" && tableModel->index(index.row(), 4).data().toInt() == tableModel->index(index.row(), 5).data().toInt() )
        {
        ...
        else if(this->windowTitle() != "Esemenyek szerkesztése")
        {
            tableModel->removeRow(index.row()); // töröljük a kijelölt sort
            tableView->setCurrentIndex(tableModel->index(index.row()-1, 0)); // beállítjuk a táblakijelölést az előző sorra
        }
        ...
        {
            QMessageBox::warning(this, trUtf8("Hiba"), trUtf8("Vannak foglalasok az esemenyre!"));
        }
